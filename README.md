...
推送后 Actions 会在每天 9:05 / 12:05 / 15:05 / 18:05 / 21:05（北京时间）各自动执行一次，也可以在 Actions 页面手动触发。签到是幂等的：只要有一次成功，当天后续运行会自动识别“今天已签”并跳过；若某个时段代理全部失效，稍后的时段会自动重试，尽量避免漏签。

## Token 过期

Token 有效期取决于服务端策略。过期后：

- 为避免一天多次 cron 重复轰炸，只有**每天首个时段（9:05）或手动触发**的运行会标红失败并发送提醒（GitHub 邮件 / SMTP），其余时段仅静音记录，因此**一天最多一封失败邮件**
