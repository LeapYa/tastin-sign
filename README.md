...
1. ✅ GitHub Actions 定时执行 —— 免费、免服务器，每天一个时段自动签到（单次运行内指数退避重试最多 5 次，兜住偶发代理失效）
...
推送后 Actions 会在每天 06:05（北京时间，对应 22:05 UTC）自动执行一次，也可以在 Actions 页面手动触发。之所以排在清晨：GitHub 定时任务常延迟数小时，排早一点让实际签到尽量落在上午。为降低被风控盯上的概率，只用这一个时段；单次运行内会做指数退避重试（最多 5 次、总时长上限约 15 分钟），以兜住偶发的免费代理失效。若最终仍失败，则直接报错（运行标红）。

## Token 过期

Token 有效期取决于服务端策略。过期后：

- 每天只有一个时段运行；若 token 过期或多次重试后仍失败，运行会**标红失败**并发送提醒（GitHub 失败邮件 / 可选 SMTP）
- Actions 日志会输出 `::error::` 高亮提醒；如果配置了 SMTP，会收到邮件通知
...
A: 这是阿里云 WAF 拦截了 GitHub Actions 的海外 IP。脚本已内置代理回退，会自动切换到国内代理重试；单次运行内还会做指数退避重试（最多 5 次、总时长上限约 15 分钟）。若重试后仍失败，则直接报错（运行标红），可在 Actions 日志查看原因。
